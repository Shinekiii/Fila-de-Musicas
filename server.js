...
// Rota para obter o estado atual do player
app.get('/api/player/status', (req, res) => {
    // Adapte conforme necessário para retornar o estado real do player
    res.json({ status: 'reproduzindo', videoId: 'dummyVideoId' });
});

// Rota para alterar o tema
app.post('/api/theme', (req, res) => {
    const { theme } = req.body;
    if (!['light', 'dark'].includes(theme)) {
        return res.status(400).json({ error: 'Tema inválido' });
    }
    // Salve o tema ou aplique a lógica necessária
    res.json({ message: `Tema alterado para ${theme}` });
});
